...
} from "./actions"; // Import UPDATE_SESSION_QUEUE
...
function reducer(state = initialState, action) {
  switch (action.type) {
    ...
    case ANSWER_CARD: {
      ...
      const updatedCard = calculateNextState(
        cardToUpdate,
        rating,
        systemSettings
      );
      ...
      return {
        ...
        currentCard:
          state.currentCard && state.currentCard.id === updatedCard.id
            ? updatedCard
            : state.currentCard,
      };
    } // Added curly braces
    ...
    case UPDATE_SESSION_QUEUE: {
      ...
      return {
        ...
        sessionState:
          newSessionQueue.length === 0 ? "finished" : state.sessionState,
      };
    } // Added curly braces
    default:
      return state;
